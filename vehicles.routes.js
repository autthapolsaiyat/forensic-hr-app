...
// Get available years from received_date
// ============================================
router.get('/years', async (req, res) => {
    try {
        const query = `
            SELECT DISTINCT EXTRACT(YEAR FROM received_date)::integer as year 
            FROM vehicles 
...
router.get('/stats', async (req, res) => {
    try {
        ...
        if (yearStart && yearEnd) {
            yearFilter = `EXTRACT(YEAR FROM received_date) BETWEEN $1 AND $2`;
            yearFilterWithWhere = `WHERE ${yearFilter}`;
            yearParams = [parseInt(yearStart), parseInt(yearEnd)];
        } else if (yearStart) {
            yearFilter = `EXTRACT(YEAR FROM received_date) >= $1`;
            yearFilterWithWhere = `WHERE ${yearFilter}`;
        ...
        if (yearFilterWithWhere) {
            yearRangeQuery = `
                SELECT 
                    MIN(EXTRACT(YEAR FROM received_date))::integer as min_year,
                    MAX(EXTRACT(YEAR FROM received_date))::integer as max_year,
                    COUNT(DISTINCT EXTRACT(YEAR FROM received_date))::integer as year_count
                FROM vehicles 
                ${yearFilterWithWhere}
            `;
...
router.get('/', async (req, res) => {
    try {
        ...
        // Year filter
        if (yearStart) {
            query += ` AND EXTRACT(YEAR FROM received_date) >= $${paramIndex}`;
            countQuery += ` AND EXTRACT(YEAR FROM received_date) >= $${paramIndex}`;
    ...
    try {
        ...
        const result = await pool.query(`
            INSERT INTO vehicles (
                ...
                received_date, vehicle_age, source, status, remarks,
                created_at, updated_at
            ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW(), NOW())
            RETURNING *
        `, [
            department_code, vehicle_type, brand, license_plate,
            received_date, vehicle_age, source, status || 'ใช้งานได้', remarks
        ]);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        `, [
            department_code, vehicle_type, brand, license_plate,
            received_date, vehicle_age, source, status, remarks, id
        ]);
